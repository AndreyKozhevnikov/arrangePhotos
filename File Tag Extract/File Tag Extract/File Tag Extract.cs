...
namespace FileTagExtract {
    ...
    public class GenerateFileInfo {
        ...
        static public void VideoFileInfo(ref List<FileTag> arr, Folder Folder, FolderItem FolderItem) {
            FileTag temp = new FileTag();
            temp.TagName = "MyDate";
            temp.TagValue=Folder.GetDetailsOf(FolderItem,208) ;
            arr.Add(temp);
            //int[] index = { 0, 1, 15, 21, 303, 304, 305, 306, 308,177, 191,208 };
            //string[] name = { "File name", "File size", "Year", "Title", "Data rate", "Frame height", "Frame rate", "Frame width", "Total Bitrate","MyDate", "MyDate2", "MyDate3" };
            //for (int i = 0; i < 12; i++) {
            //    temp.TagName = name[i];
            //    temp.TagValue = Folder.GetDetailsOf(FolderItem, index[i]).ToString();
            //    arr.Add(temp);
